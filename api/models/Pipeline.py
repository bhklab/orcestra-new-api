...
class Zenodo(BaseModel):
    ...
    creators: List[dict[str]] = Field(default_factory = lambda: [{"name": "Haibe-Kains, Benjamin",
             "orcid": '0000-0002-7684-0079',
             "type": "ContactPerson"}], merge = True)
    
    dataset_type: str
    description: str = " generated by ORCESTRA. Metadata can be found on https://orcestra.ca"


    @model_validator(mode="after")
    def merge_defaults(self):
        # Prepend dataset type to description
        self.description = self.dataset_type + self.description
        #Merge default values with user provided values for fields with 'merge' metadata
        for name, field in self.model_fields.items():
            
            if field.metadata and "merge" in field.metadata:
                defaults = deepcopy(field.default)
                current = getattr(self, name)
                setattr(self, name, defaults + current)

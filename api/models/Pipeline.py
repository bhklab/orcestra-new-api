...
class Zenodo(BaseModel):
    ...
    upload_files: List[str] = Field(default_factory = lambda: [], json_schema_extra = {"not_metadata": True})
    ...
    async def zenodo_upload(self) -> dict[str, str]:
        ...
        if not operation or not update_existing:
            ...
            files = [str(path / p) for p in self.upload_files]
        ...
        zenodo_entry["files_uploaded"] = self.upload_files if not operation or not update_existing else existing_zenodo_entry["files_uploaded"]

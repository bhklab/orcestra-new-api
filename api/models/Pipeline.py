...
class Zenodo(BaseModel):
    ...
    def _structure_metadata(self) -> dict:
        ...
        for name, field_info in self.__dict__.items():
                
                if name not in ["pipeline_name", "dataset_type"]:
                    metadata[name] = field_info
        ...
        print(self.__private_attributes__)

        return metadata
    
    
    async def zenodo_upload(self):
